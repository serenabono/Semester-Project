    ...
    for el1 in rangex:
        for el2 in rangey:
            for el in rangea:
                ...
                img_out[:,:,0:3]=img_out[:,:,0:3]+np.repeat(np.repeat(np.linspace(1,0,img_out.shape[1]),3),img.shape[0]).reshape(img_out[:,:,0:3].shape)
