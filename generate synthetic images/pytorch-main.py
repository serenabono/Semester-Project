...
def runSimulate(K, dx, thetax, dy, thetay, dz, thetaz):
    ...
    R_f = torch.matmul(torch.matmul(Rx, Ry), Rz)
    R_ff = torch.transpose(torch.cat((torch.transpose(R_f,1,0), torch.unsqueeze(torch.tensor([0,0,0]),dim=0)),dim=0),1,0)
...
def main():
    ...
    frontal = True
    im_path = 'C:/Users/super/Desktop/robocup/robocup_thicker.jpeg'
