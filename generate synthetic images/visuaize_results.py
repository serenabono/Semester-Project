...
def main():
    #realtxt = open('dataset_test.txt', 'r')
    ...
    for i, fake in enumerate(faketxt.readlines()):
        ...
        #H_real = runSimulate(K,float(real_split[0]),float(real_split[1]),float(real_split[2]),float(real_split[3]),float(real_split[4]),float(real_split[5]),float(real_split[6]))
        ...
        #img_out_real = simImage(img, H_real)
