...
class cycleGAN(object):
    ...
    def train(self,args):
        ...
        def get_weights(image):
            weight_map=image.detach().clone()
            weight_map=((weight_map+1)/2)*255
            weight_map=(np.log((weight_map[0,0]*weight_map[0,1])+1)+1)/10
            return weight_map

        #green [  0.0000, 137.0000,  39.0000]->log(0+1)+1  ->1
        #black [0,  0,  0]->log(0+1)+1  ->1
        #grey [128, 128,  128]->log(16384+1)+1 ->9.7
        #white [17, 140,  51]->log(1960+1)+1 ->7.5
        ...
        for epoch in range(self.start_epoch, args.epochs):
            ...
            for i, (a_real, b_real) in enumerate(zip(a_loader, b_loader)):
                ...
                weights_b_real=get_weights(b_real)
                ...
                # Identity losses
                ###################################################
                b_idt_loss = (weights_b_real.to("cuda")*self.L1(b_idt, b_real) * args.lamda * args.idt_coef).mean()
                a_idt_loss = (self.L1(a_idt, a_real) * args.lamda * args.idt_coef).mean()
                ...
                ###################################################
                a_cycle_loss = (self.L1(a_recon, a_real) * args.lamda).mean()
                b_cycle_loss = (weights_b_real.to("cuda")*self.L1(b_recon, b_real) * args.lamda).mean()
